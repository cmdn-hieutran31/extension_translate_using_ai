...
async function translateWithMyMemory(text, targetLang) {
    ...
    try {
        const response = await fetchWithTimeout(
            ...
            { timeout: 3000 } // 3 seconds timeout for MyMemory
        );
...
async function checkDictionaryWithGoogle(text, targetLang) {
    ...
    try {
        ...
        const response = await fetchWithTimeout(url, { timeout: 2000 }); // 2 seconds for Dictionary
            ...
            try {
                ...
                if (request.forceAI) {
                    ...
                    if (!apiKey) {
                        sendResponse({
                            success: false,
                            error: 'Translation services failed and Gemini API Key not found for fallback.',
                        });
                        return;
                    }
