...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const { savedSourceText, savedTranslation } = await chrome.storage.local.get(['savedSourceText', 'savedTranslation']);
  ...
  try {
    ...
    if (chrome.scripting && chrome.scripting.executeScript) {
      try {
        ...
        console.log('executeScript failed, trying message method:', scriptError.message);
      }
    }
...
translateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const translation = await translateWithMyMemory(text, targetLang.value);
...
async function translateWithMyMemory(text, targetLang) {
  ...
  // Detect source language (simplified - just assume English if not detected)
  const sourceLang = 'autodetect'; // MyMemory will auto-detect
  ...
  // Fallback: if quota exceeded, try to return original text with note
  if (data.responseStatus === 403 || data.responseStatus === 429) {
    throw new Error('MyMemory API quota exceeded. Please try again later or use AI translation.');
  }
...
async function trackMyMemoryUsage(charsUsed) {
  try {
    const data = await chrome.storage.local.get(['myMemoryChars', 'myMemoryLastDate']);
...
retranslateBtn.addEventListener('click', async () => {
  ...
  translationResult.innerHTML = '<span class="placeholder">AI Translating...</span>';
  showStatus('Translating with AI...');

  try {
    const translation = await translateText(text, targetLang.value, apiKey);
    translationResult.textContent = translation;
...
clearBtn.addEventListener('click', async () => {
  sourceText.value = '';
  translationResult.innerHTML = '<span class="placeholder">Translation will appear here...</span>';
  grammarResult.innerHTML = '<span class="placeholder">Grammar check results will appear here...</span>';
...
async function extractTextFromImage(imageData) {
  ...
  extractedText.innerHTML = '<span class="placeholder">Extracting text from image...</span>';
  ...
  try {
    ...
    if (text && text.trim()) {
      ...
      extractedText.innerHTML = '<span class="placeholder" style="color: #f59e0b;">No text could be extracted from this image.</span>';
